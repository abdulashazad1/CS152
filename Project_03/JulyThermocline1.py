...
def clearfile(fileToClear):
    ...
    fp.close()
clearfile('JulyThermocline1.csv')

#name function
fp = open('JulyThermocline1.csv', 'a') #opens the file 'JulyThermocline.csv' to append (write) to it
fp.write('Day' + ',' + 'Thermodepth 12:00 PM' + '\n') #Write the header in that file
...
def main(time):
    ...
    for line in fp: #for loop for that loops through every line in fp
        ...
        if time in words[0]: #conditional checking for the time at which the data was taken
            ...
            fp = open('JulyThermocline1.csv', 'a') #opens the file JulyThermocline.csv' to append to it
